...
{
    public abstract class BaseIntRepository<T> : IBaseIntRepository<T>
        where T : BaseIntEntity
    {
        ...
        protected BaseIntRepository(DbCatalog _context)
        {
        ...
        public List<T> GetPage(IList<Expression<Func<T, bool>>> filters, PagingIntParameters<T> pagingParams)
        {
    ...
    public class PagingIntParameters<T> where T : BaseIntEntity
    {
